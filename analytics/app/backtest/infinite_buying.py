...
def run_infinite_buying(
    ...
    states: dict[str, _AssetState] = {
        t: _AssetState(cash_alloc=per_asset_capital) for t in tickers
    }

    equity_history: list[tuple[pd.Timestamp, float]] = []
    daily_budget: dict[str, float] = {t: per_asset_capital / p.split for t in tickers}
