...
│   │   ├── infinite_buying.py  무한매수법(laoer/yeonri)
...
├── verify_yeonri_may.py         연리무한매수법 5월 실거래 검증 스크립트
└── requirements.txt
```
...
1. **익절**: `high(또는 close) ≥ avg×(1+tp/100)` 이면 `qty−leave_shares` 매도 → cycle 리셋, `cycles_completed++`. `compound=True` 면 잔여현금 재분할. `restart_buy_fraction>0`(연리) 이면 익절 직후 보통가로 `cycle_budget×fraction` 즉시 재매수(평단 재기준 = 사다리타기).
...
**variant 비교**: `laoer`=익절 시 전량 매도(leave_shares=0, compound=True). `yeonri`=`leave_shares=1, compound=False, restart_buy_fraction=0.5, take_profit_pct=13, loc_offset_pct=10, split=40` (main.py `YEONRI_DEFAULTS`). TQQQ:SOXL 가중 미지정 시 실거래 검증값 73:27 자동 적용.
...
- `predict_signal_for_yeonri(df, ticker, strong_down_threshold=0.38)` → `NO_MODEL | SKIP_LOC_LARGE | ALLOW_LOC_LARGE` (무한매수법 XGBoost 오버레이 게이트).
...
| POST | `/backtest/infinite-buying` | 무한매수법(laoer/yeonri, ticker_weights…) |
...
| 연리 프리셋 | split40·TP13·offset10·leave1·restart0.5 | `YEONRI_DEFAULTS` |
