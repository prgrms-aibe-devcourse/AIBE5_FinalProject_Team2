 ...
 *
 * <p>비용 통제: <b>STANDARD 이상</b>만 대표 LIVE 전략을 하루 <b>2~4회(등급별)</b> 자동 브리핑한다.
...
public class BriefingQuotaPolicy {
    ...
    @Value("${app.briefing.daily.standard:2}") private int standard;  // 2회/일
    @Value("${app.briefing.daily.premium:3}")  private int premium;   // 3회/일
    @Value("${app.briefing.daily.expert:4}")   private int expert;    // 4회/일 (소진 후엔 Gemini 간략 브리핑)
