 ...
 *  - GET    /api/user/access                현재 사용자 Developer IDE 접근 정보(게이팅 UI용)
...
public class UserApiKeyController {
    ...
    public ResponseEntity<?> save(@PathVariable String provider, @RequestBody Map<String, String> body) {
        ...
        // BYOK(Claude) 연동은 Developer IDE 권한(STANDARD+/allowlist) 필요.
        if (!access.canUseDeveloper(uid)) {
            return ResponseEntity.status(HttpStatus.PAYMENT_REQUIRED).body(Map.of(
                    "error", "Developer IDE(본인 Claude 키 연동)는 STANDARD 구독부터 사용할 수 있습니다.",
