...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        try { ar = dataCleanupService.cleanupPartners(); }
        catch (Exception e) { log.warn("[DataSeeder] partners cleanup 실패: {}", e.getMessage()); }
        try { op = dataCleanupService.cleanupOrphans(); }
        catch (Exception e) { log.warn("[DataSeeder] orphan cleanup 실패: {}", e.getMessage()); }
        log.info("[DataSeeder] 데이터 정리: projects={}, clients={}, partners={}, orphans={} 삭제됨", pr, cr, ar, op);
