...
function BigStat({ label, value, unit = "", theme, positive, negative, hint }) {
  ...
  return (
    <div className="bigstat-card" style={{ padding: "14px 16px", background: theme.codeBg, borderRadius: 10,
      display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, minHeight: 64,
      minWidth: 0, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 5, minWidth: 0, overflow: "hidden" }}>
        <span style={{ fontSize: "clamp(10px, 2cqw, 16px)", fontWeight: 700, color: theme.textMuted }}>{label}</span>
        <InfoDot hint={hint} />
      </div>
      <div style={{ fontSize: "clamp(13px, 3.1cqw, 25px)", fontWeight: 900, color, fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap", letterSpacing: -0.5 }}>{v}{unit}</div>
    </div>
  );
}
    ...
    <div>
      ...
      {bt && (
        <>
          ...
          <div data-tutorial-id="tutorial-backtest-chart">
          {/* 차트 탭 (자산곡선 + 종목별) */}
