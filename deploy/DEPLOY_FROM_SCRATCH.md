...
```bash
mysql -u devbridge -p'DevBridge2026!' devbridge_db < /tmp/devbridge_db_full_dump.sql

# 검증 (32개 테이블 + 핵심 row 수)
mysql -u devbridge -p'DevBridge2026!' devbridge_db -e "
SHOW TABLES;
SELECT 'users' AS t, COUNT(*) FROM users
UNION ALL SELECT 'projects', COUNT(*) FROM projects
UNION ALL SELECT 'partner_review', COUNT(*) FROM partner_review
UNION ALL SELECT 'client_review', COUNT(*) FROM client_review;
DESCRIBE partner_review;
"
```

`partner_review`에 `expertise / schedule / communication / proactivity` 4개 컬럼이 보여야 `validate` 통과합니다 (이번 dump에는 이미 포함).
